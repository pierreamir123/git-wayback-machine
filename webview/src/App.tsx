...
function App() {
    ...
    const handleMessage = (event: MessageEvent) => {
      ...
      switch (message.command) {
        case 'setHistory':
          ...
          // Auto-select latest commit if none selected
  ...
  const handleStep = useCallback((offset: number) => {
    if (!history) return;
    // Note: offset 1 means "newer" (lower index), -1 means "older" (higher index)
  ...
  useEffect(() => {
    if (isPlaying) {
      playTimerRef.current = window.setInterval(() => {
        handleStep(1); // Play forward in time (newer commits)
      }, 2000 / replaySpeed);
    } else {
  ...
  return (
    <div className="flex h-screen bg-transparent text-gray-100 select-none overflow-hidden">
      {/* Sidebar: Timeline */}
      <div className="w-80 border-r border-white/10 flex flex-col p-4 bg-black/10">
        {history ? (
          <Timeline 
      ...
      <div className="flex-1 flex flex-col p-4 overflow-hidden relative">
        {fileContent ? (
          <div className="flex flex-col h-full space-y-4">
            ...
            <FileViewer content={fileContent.content} />
